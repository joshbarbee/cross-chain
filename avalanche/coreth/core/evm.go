...
// Transfer subtracts amount from sender and adds amount to recipient using the given Db
func TransferMultiCoin(db vm.StateDB, sender, recipient common.Address, coinID common.Hash, amount *big.Int, prefetch bool, depth int) {
	db.SubBalanceMultiCoin(sender, coinID, amount)
	db.AddBalanceMultiCoin(recipient, coinID, amount)

	if !prefetch {
		mgologger.AddTransferLog(sender.String(), recipient.String(), coinID.String(), amount.String(), depth, "ETH")
	}
}
